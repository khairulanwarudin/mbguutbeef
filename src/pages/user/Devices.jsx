...
import { Plus, Cpu, Trash2, Settings } from 'lucide-react';
...
const Devices = () => {
  const [devices, setDevices] = useState([
    { 
      id: 'esp-1', 
      deviceId: 'TERMO_ESP32_01', 
      name: 'ESP32 Ruang Tamu', 
      status: 'online',
      thresholds: { minTemp: 15, maxTemp: 35, minHum: 40, maxHum: 80 }
    }
  ]);
  ...
  const handleAddDevice = (e) => {
    ...
    if (newDeviceId && newName) {
      setDevices([...devices, {
        ...
        name: newName,
        status: 'offline',
        thresholds: { minTemp: 10, maxTemp: 40, minHum: 30, maxHum: 90 } // default
      }]);
      setNewDeviceId('');
      setNewName('');
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: 'Hapus Alat?',
      text: "Data histori alat ini juga mungkin akan terhapus.",
      icon: 'warning',
  ...
  const openConfig = (device) => {
    setEditThresholds(device.thresholds);
    setShowConfig(device.id);
  };

  const saveConfig = (id) => {
    setDevices(devices.map(d => d.id === id ? { ...d, thresholds: editThresholds } : d));
    setShowConfig(null);
    Swal.fire({
      icon: 'success',
      title: 'Tersimpan',
            ...
            <div key={device.id} className="glass-card" style={{ padding: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 style={{ margin: '0 0 0.25rem 0' }}>{device.name}</h4>
